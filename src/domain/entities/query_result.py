...
class EntityResult(BaseModel):
    ...
    reference_id: Optional[dict] = Field(default=None, description="Reference identifier for citations")
...
class KeywordsInfo(BaseModel):
    ...
    high_level: List[str] = Field(default_factory=list, description="High-level keywords extracted")
    low_level: List[str] = Field(default_factory=list, description="Low-level keywords extracted")


class ProcessingInfo(BaseModel):
    """Processing statistics for the query."""
    total_entities_found: int = Field(default=0, description="Total entities before truncation")
    total_relations_found: int = Field(default=0, description="Total relations before truncation")
    entities_after_truncation: int = Field(default=0, description="Entities after token truncation")
    relations_after_truncation: int = Field(default=0, description="Relations after token truncation")
    merged_chunks_count: int = Field(default=0, description="Chunks before final processing")
